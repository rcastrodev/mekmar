...
class ProductController extends Controller
{
    ...
    public function escalasYSacabocados()
    {
        $product    = Product::where('name', 'ESCALAS Y SACABOCADOS')->first();
        $category   = Category::where('name', 'ESCALAS Y SACABOCADOS')->first();

        $numberOfImagesAllowed = 3;
        if (count($product->images)) 
            $numberOfImagesAllowed = 3 - $product->images()->count();
        
        
        return view('administrator.product.escalas', compact('product', 'category', 'numberOfImagesAllowed'));
    }
}
